...
export default function Params ({ ast, pattern, params, locale, onChange }) {
  ...
  let final = ''
  let textDir = 'ltr'
  let rtlLocales = ['ar', 'he', 'fa', 'ur', 'ps'] // list's not exhaustive.
  
  if (rtlLocales.indexOf(locale) != -1){
  	textDir = 'rtl'
  }
  
  try { final = new MessageFormat(pattern, locale).format(args) }
  ...
  return (
    <fieldset className='TryIt-params'>
      ...
      <span className='TryIt-final' dir={ textDir }>{ final }</span>
    </fieldset>
  )
}
